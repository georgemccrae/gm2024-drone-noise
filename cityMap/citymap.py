...
class CityMap:
    ...
    def __init__(self, left: float, right: float, bottom: float, top: float):
        ...
        # Convert Population Density column to numeric
        self.pd_data['Population_Density_in_2010'] = pd.to_numeric(self.pd_data['Population_Density_in_2010'], errors='coerce')

        # Calculate sum and probabilities
        pd_sum = self.pd_data['Population_Density_in_2010'].sum()
        
        # Check if pd_sum is not zero to avoid division by zero
        if pd_sum > 0:
            self.pd_prob = self.pd_data['Population_Density_in_2010'] / pd_sum
        else:
            self.pd_prob = [0] * len(self.pd_data)  # Handle the case where the sum is zero
